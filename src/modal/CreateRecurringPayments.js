...
import GetPaymentDetails from "src/components/StepForm/GetPaymentDetails";

import { MoneyStreamingContext } from "src/context/CreateMoneyStreamContext";

const steps = ["Select Chain", "Payment Details"];

export default function CreateRecurringPayments(props) {
      ...
      <Dialog open={props.op} onClose={props.close} fullWidth>
        <DialogContent style={{ overflowX: "hidden" }}>
          <Box sx={{ width: "100%" }}>
            ...
            {activeStep === 0 && <GetChain />}
            {activeStep === 1 && <GetPaymentDetails />}
